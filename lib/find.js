...
module.exports = {
  activate(param) {
    ...
    return this.subscriptions.add(
      lumine.commands.add("lumine-workspace", {
        "search-panel:select-next": (event) => selectNextObjectForEvent(event)?.findAndSelectNext(),
        "search-panel:select-all": (event) => selectNextObjectForEvent(event)?.findAndSelectAll(),
        "search-panel:select-undo": (event) => selectNextObjectForEvent(event)?.undoLastSelection(),
        "search-panel:select-skip": (event) =>
          selectNextObjectForEvent(event)?.skipCurrentSelection(),
      }),
    );
  },
