...
const { TextEditor, Disposable, CompositeDisposable } = require("lumine");
...
module.exports = class ProjectFindView {
  constructor(
    ...
    this.subscriptions.add(
      lumine.textEditors.add(this.refs.findEditor, { role: "input" }),
      lumine.textEditors.add(this.refs.replaceEditor, { role: "input" }),
      lumine.textEditors.add(this.refs.pathsEditor, { role: "input" }),
      lumine.config.observe("core.excludeVcsIgnoredPaths", (newValue) => {
        this.model.getFindOptions().set({ excludeVcsIgnoredPaths: newValue });
      }),
    );
      ...
      $.header(
        ...
        $.span(
          ...
          $.span(
            ...
            $.button(
              {
                ref: "includeIgnoredNamesOptionButton",
                className: "btn option-include-ignored-names",
              },
              $.svg({
                className: "icon",
  ...
  didShow() {
    lumine.views.getView(lumine.workspace).classList.add("find-visible");
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      lumine.tooltips.add(this.refs.closeButton, {
        title: 'Close Panel <span class="keystroke">Esc</span>',
        html: true,
      }),

      lumine.tooltips.add(this.refs.regexOptionButton, {
        title: "Use Regex",
        keyBindingCommand: "search-panel:project-toggle-regex-option",
        keyBindingTarget: this.findEditor.element,
      }),

      lumine.tooltips.add(this.refs.caseOptionButton, {
        title: "Match Case",
        keyBindingCommand: "search-panel:project-toggle-case-option",
        keyBindingTarget: this.findEditor.element,
      }),

      lumine.tooltips.add(this.refs.wholeWordOptionButton, {
        title: "Whole Word",
        keyBindingCommand: "search-panel:project-toggle-whole-word-option",
        keyBindingTarget: this.findEditor.element,
      }),

      lumine.tooltips.add(this.refs.includeIgnoredNamesOptionButton, {
        title: "Include Ignored Names",
        keyBindingCommand: "search-panel:project-toggle-include-ignored-names-option",
        keyBindingTarget: this.findEditor.element,
      }),

      lumine.tooltips.add(this.refs.includeVcsIgnoredPathsOptionButton, {
        title: "Include VCS-Ignored Files",
        keyBindingCommand: "search-panel:project-toggle-include-vcs-ignored-paths-option",
        keyBindingTarget: this.findEditor.element,
      }),

      lumine.tooltips.add(this.refs.findAllButton, {
        title: "Find All",
        keyBindingCommand: "search-panel:search",
        keyBindingTarget: this.findEditor.element,
      }),

      lumine.tooltips.add(this.refs.pcre2OptionButton, {
  ...
  didHide() {
    ...
    if (lumine.config.get("search-panel.cleanOnHide")) {
  ...
  handleEvents() {
    this.subscriptions.add(
      lumine.commands.add("lumine-workspace", {
        // The same two names the find panel registers, so they carry the same
        // sentence: which of the two the palette shows depends on which panel
        // is open, and two spellings would make that visible.
        "search-panel:use-selection-as-find-pattern": {
          description: "Put the editor selection into the find field.",
          didDispatch: () => this.setSelectionAsFindPattern(),
        },
        "search-panel:use-selection-as-replace-pattern": {
          description: "Put the editor selection into the replace field.",
          didDispatch: () => this.setSelectionAsReplacePattern(),
        },
      }),
    );
    ...
    this.subscriptions.add(
      lumine.commands.add(this.element, {
        "search-panel:focus-next": {
          description: "Move focus to the next field of the panel.",
          didDispatch: () => this.focusNextElement(1),
        },
        "search-panel:focus-previous": {
          description: "Move focus to the previous field of the panel.",
          didDispatch: () => this.focusNextElement(-1),
        },
        "core:confirm": () => this.confirm(),
        "core:close": () => this.panel && this.panel.hide(),
        "core:cancel": () => this.panel && this.panel.hide(),
        "search-panel:project-confirm": {
          description: "Run the search across every file the panel covers.",
          didDispatch: () => this.confirm(),
        },
        "search-panel:project-toggle-regex-option": {
          description: "Read the find pattern as a regular expression.",
          didDispatch: () => this.toggleRegexOption(),
        },
        "search-panel:project-toggle-case-option": {
          description: "Require the match to have the case the pattern is written in.",
          didDispatch: () => this.toggleCaseOption(),
        },
        "search-panel:project-toggle-whole-word-option": {
          description: "Match whole words only, never a part of a longer one.",
          didDispatch: () => this.toggleWholeWordOption(),
        },
        "search-panel:project-toggle-include-ignored-names-option": {
          description: "Search names excluded by the core and package ignore lists as well.",
          didDispatch: () => this.toggleIncludeIgnoredNamesOption(),
        },
        "search-panel:project-toggle-include-vcs-ignored-paths-option": {
          description: "Search the files the repository ignores as well.",
          didDispatch: () => this.toggleIncludeVcsIgnoredPathsOption(),
        },
        "search-panel:project-replace-all": {
          description: "Replace every match in every file the search found.",
          didDispatch: () => this.replaceAll(),
        },
        "search-panel:clear": {
          description: "Empty the find field and drop the match highlights.",
          didDispatch: () => this.clear(),
        },
      }),
    );
    ...
    let afterSearch = () => {
      if (lumine.config.get("search-panel.closeFindPanelAfterSearch")) {
    ...
    this.refs.includeIgnoredNamesOptionButton.addEventListener("click", () =>
      this.toggleIncludeIgnoredNamesOption(),
    );
  ...
  async replaceAll() {
    ...
    if (findPattern && findPattern !== currentPattern) {
      await lumine.window.confirm({
        ...
        detail: `Please run the search with the new pattern '${currentPattern}' before running a replace-all`,
        buttons: ["OK"],
      });
      return;
    }
    ...
    } ${pluralize(this.model.matchCount, "time")} in ${this.model.pathCount} ${pluralize(
      this.model.pathCount,
      "file",
    )}`;
    const buttonChosen = await lumine.window.confirm({
  ...
  findInCurrentlySelectedDirectory(selectedElement) {
    ...
    if (absolutePath) {
      let [rootPath, relativePath] = lumine.project.relativizePath(absolutePath);
  ...
  showResultPane() {
    ...
    let openDirection = lumine.config.get("search-panel.projectSearchResultsPaneSplitDirection");
    if (openDirection !== "none") {
      options.split = openDirection;
    }
    return lumine.workspace.open(ResultsPaneView.URI, options);
  }

  onFinishedReplacing(results) {
    if (!results.replacedPathCount) lumine.notifications.beep();
  ...
  updateReplaceAllButtonEnablement(results) {
    ...
    if (canReplace) {
      ...
      this.replaceTooltipSubscriptions.add(
        lumine.tooltips.add(this.refs.replaceAllButton, {
          title: "Replace All",
  ...
  updateSyntaxHighlighting() {
    if (this.model.getFindOptions().useRegex) {
      this.findEditor.setGrammar(lumine.grammars.grammarForScopeName("source.regexp"));
      return this.replaceEditor.setGrammar(
        lumine.grammars.grammarForScopeName("source.regexp.replacement"),
      );
    } else {
      this.findEditor.setGrammar(lumine.grammars.nullGrammar);
      return this.replaceEditor.setGrammar(lumine.grammars.nullGrammar);
    }
  }
  ...
  updateOptionsLabel() {
    ...
    if (!this.model.getFindOptions().useCoreIgnoredNames) {
      label.push("Ignored Names");
    }

    if (!this.model.getFindOptions().excludeVcsIgnoredPaths) {
  ...
  updateOptionButtons() {
    ...
    this.setOptionButtonState(
      this.refs.includeIgnoredNamesOptionButton,
      !this.model.getFindOptions().useCoreIgnoredNames,
    );
    this.setOptionButtonState(
      this.refs.includeVcsIgnoredPathsOptionButton,
      !this.model.getFindOptions().excludeVcsIgnoredPaths,
    );
  ...
  updateEngineOptionButtons() {
    this.setOptionButtonState(
      ...
      lumine.config.get("search-panel.enablePCRE2"),
    );
  }
  ...
  toggleIncludeVcsIgnoredPathsOption() {
    this.search({
      onlyRunIfActive: true,
      excludeVcsIgnoredPaths: !this.model.getFindOptions().excludeVcsIgnoredPaths,
    });
  }

  toggleIncludeIgnoredNamesOption() {
    this.search({
      onlyRunIfActive: true,
      useCoreIgnoredNames: !this.model.getFindOptions().useCoreIgnoredNames,
    });
  }

  togglePCRE2Option() {
    const newValue = !lumine.config.get("search-panel.enablePCRE2");
    lumine.config.set("search-panel.enablePCRE2", newValue);
    this.updateEngineOptionButtons();
    return this.search({ onlyRunIfActive: true });
  }
