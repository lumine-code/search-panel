...
module.exports = class ResultsPaneView {
  ...
  setModel(model) {
    ...
    const resultsViewUpdate = this.refs.resultsView.update({
      model,
      collapseNewResults: this.collapseNewResults,
    });
    const paneUpdate = this.onFinishedSearching(model.getResultsSummary());
    return Promise.all([resultsViewUpdate, paneUpdate]);
  }
  ...
  destroy() {
    ...
    this.refs.resultsView?.destroy();
    this.model.setActive(false);
    this.modelSubscriptions.dispose();
    this.subscriptions.dispose();
    if (this.separatePane) {
      ResultsPaneView.destroyResultsModel?.(this.model);
      this.model = null;
    }
  }
  ...
  beginWindowSurfaceTransition() {
    ...
    this.refs.resultsView?.prepareWindowSurfaceTransition();
    const finish = () => {
      if (this.destroyed) return;
      return this.refs.resultsView?.finishWindowSurfaceTransition();
    };
  ...
  searchContextLineCountChanged() {
    this.searchContextLineCountBefore = lumine.config.get(
      "search-panel.searchContextLineCountBefore",
    );
    this.searchContextLineCountAfter = lumine.config.get(
      "search-panel.searchContextLineCountAfter",
    );
