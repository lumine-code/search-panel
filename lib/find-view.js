const { TextEditor, CompositeDisposable } = require("lumine");
...
module.exports = class FindView {
  ...
  didShow() {
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      lumine.tooltips.add(this.refs.selectionOptionButton, {
  ...
  handleEvents() {
    ...
    this.refs.replaceNextButton.addEventListener("click", (e) =>
      e.shiftKey
        ? this.replacePrevious({ focusEditorAfter: true })
        : this.replaceNext({ focusEditorAfter: true }),
    );
    this.refs.replaceAllButton.addEventListener("click", this.replaceAll.bind(this));
    // Wrapped rather than bound: these take an options object, and a bound
    // handler would hand them the CommandEvent as one. The editor gets focus
    // back afterwards, the way find-next does — the command is dispatched from
    // wherever focus already is, so pulling it into the panel is never wanted.
    this.subscriptions.add(
      lumine.commands.add("lumine-workspace", {
        "search-panel:replace-previous": {
          description: "Replace this match and move back to the previous one.",
          didDispatch: () => this.replacePrevious({ focusEditorAfter: true }),
        },
        "search-panel:replace-next": {
          description: "Replace this match and move on to the next one.",
          didDispatch: () => this.replaceNext({ focusEditorAfter: true }),
        },
        "search-panel:replace-current": {
          description: "Replace this match and stay where it was.",
          didDispatch: this.replaceCurrent.bind(this),
        },
        "search-panel:replace-all": {
          description: "Replace every match in the file at once.",
          didDispatch: this.replaceAll.bind(this),
        },
      }),
    );

    this.subscriptions.add(
      lumine.commands.add(this.findEditor.element, {
        "core:confirm": () => this.confirm(),
        "search-panel:confirm": () => this.confirm(),
        "search-panel:show-previous": {
          description: "Put the previous search back in the find field.",
          didDispatch: () => this.showPrevious(),
        },
      }),
    );

    this.subscriptions.add(
      lumine.commands.add(this.replaceEditor.element, {
        // Confirming from the replace field keeps the caret there, so a run of
        // replacements is a run of enters.
        "core:confirm": () => this.replaceNext({ fieldToFocus: this.replaceEditor }),
      }),
    );
    ...
    this.subscriptions.add(
      lumine.commands.add(this.element, {
        ...
        "search-panel:focus-next": {
          ...
          didDispatch: this.toggleFocus.bind(this),
        },
        "search-panel:focus-previous": {
          description: "Move focus to the previous field of the panel.",
          didDispatch: this.toggleFocus.bind(this),
        },
        "search-panel:toggle-regex-option": {
          description: "Read the find pattern as a regular expression.",
          didDispatch: this.toggleRegexOption.bind(this),
        },
        "search-panel:toggle-case-option": {
          description: "Require the match to have the case the pattern is written in.",
          didDispatch: this.toggleCaseOption.bind(this),
        },
        "search-panel:toggle-selection-option": {
          description: "Search only inside what is selected in the editor.",
          didDispatch: this.toggleSelectionOption.bind(this),
        },
        "search-panel:toggle-whole-word-option": {
          description: "Match whole words only, never a part of a longer one.",
          didDispatch: this.toggleWholeWordOption.bind(this),
        },
        "search-panel:clear": {
          description: "Empty the find field and drop the match highlights.",
          didDispatch: this.clear.bind(this),
        },
      }),
    );
  ...
  confirm() {
    this.findNext({
      focusEditorAfter: lumine.config.get("search-panel.focusEditorAfterSearch"),
    });
  }
  ...
  liveSearch() {
    ...
    if (
      ...
      (findPattern.length >= lumine.config.get("search-panel.liveSearchMinimumCharacters") &&
  ...
  replaceNext(options = { focusEditorAfter: true }) {
    this.replace("next", options);
  }

  replacePrevious(options = { focusEditorAfter: true }) {
    this.replace("previous", options);
  }
  ...
  replace(direction, options = { focusEditorAfter: true }) {
    this.search();
    this.findHistoryCycler.store();
    this.replaceHistoryCycler.store();
    ...
    if (this.target.canReplace() && this.target.getResultCount() > 0) {
      ...
      this.navigate(direction, options);
    } else {
      lumine.notifications.beep();
    }
  }
  ...
  updateResultsUi(results, { scrollToLiveResult = false } = {}) {
    ...
    if (this.target.getFindOptions().findPattern) {
      ...
      if (
        ...
        results > 0 &&
        lumine.config.get("search-panel.scrollToResultOnLiveSearch")
      ) {
  ...
  updateSyntaxHighlighting() {
    if (this.target.getFindOptions().useRegex) {
      ...
      this.replaceEditor.setGrammar(lumine.grammars.nullGrammar);
    }
  }
  ...
  updateFindEnablement() {
    ...
    if (usable && (!hadFindTooltip || isDisabled)) {
      ...
      this.findTooltipSubscriptions.add(
        lumine.tooltips.add(this.refs.nextButton, {
          title: "Find Next",
  ...
  updateReplaceEnablement() {
    ...
    if (canReplace) {
      ...
      this.replaceTooltipSubscriptions.add(
        lumine.tooltips.add(this.refs.replaceNextButton, {
          title: "Replace Next",
  ...
  showWrapIcon(icon) {
    if (!lumine.config.get("search-panel.showSearchWrapIcon")) return;
