...
const { Emitter, TextEditor, Range } = require("lumine");
...
module.exports = class ResultsModel {
  constructor(findOptions) {
    ...
    lumine.workspace.getCenter().observeActivePaneItem((item) => {
      if (item instanceof TextEditor && lumine.project.contains(item.getPath())) {
  ...
  shouldAddResult(filePath) {
    ...
    return lumine.workspace.filePathMatchesPatterns(filePath, searchPaths);
  }
