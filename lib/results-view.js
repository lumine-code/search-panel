...
module.exports = class ResultsView {
  constructor({ model, collapseNewResults = false }) {
    ...
    this.subscriptions = new CompositeDisposable(
      lumine.config.observe("editor.fontFamily", this.fontFamilyChanged.bind(this)),
      ...
      lumine.commands.add(this.element, {
        ...
        "search-panel:copy-path": {
          description: "Copy the path of the file holding the selected result.",
          didDispatch: this.copyPath.bind(this),
        },
        "search-panel:open-in-new-tab": {
          description: "Open the selected result in a tab that outlives the search.",
          didDispatch: this.openInNewTab.bind(this),
        },
      }),
    );
  }
  ...
  rebuildRows() {
    this.resultRowGroups = Object.values(this.model.results).map(
  ...
  update({ model, collapseNewResults = false }) {
    this.collapseNewResults = collapseNewResults;
    this.setModel(model);
  ...
  bindResizeObserver() {
    ...
    if (!domWindow) throw new Error("A search results view requires a live owner Window");
    if (this.resizeObserver && this.resizeObserverWindow === domWindow) return;

    this.disconnectResizeObserver();
    this.resizeObserverWindow = domWindow;
    this.resizeObserver = new domWindow.ResizeObserver(() => this.invalidateItemHeights());
  ...
  prepareWindowSurfaceTransition() {
    this.disconnectResizeObserver();
    this.refs.listView?.prepareWindowSurfaceTransition();
  }

  async finishWindowSurfaceTransition() {
    this.bindResizeObserver();
    await this.refs.listView?.finishWindowSurfaceTransition();
    await this.invalidateItemHeights();
  }
  ...
  async confirmResult({ pending } = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await lumine.workspace.open(selectedRow.group.result.filePath, {
        pending,
        searchAllPanes: true,
        split:
          reverseDirections[
            lumine.config.get("search-panel.projectSearchResultsPaneSplitDirection")
          ],
      });
      // An open can decline — a large file the reader cancelled, a path that
      // cannot be read, a workspace center that is full — and then there is no
      // editor to reveal the match in.
      if (!editor) return;
  ...
  copyResult() {
    ...
    if (selectedRow.data.matches) {
      ...
      lumine.clipboard.write(selectedRow.data.matches[0].lineText);
    }
  }
  ...
  copyPath() {
    ...
    let [projectPath, relativePath] = lumine.project.relativizePath(filePath);
    if (projectPath && lumine.project.getDirectories().length > 1) {
      relativePath = path.join(path.basename(projectPath), relativePath);
    }
    lumine.clipboard.write(relativePath);
  }
  ...
  toggleLeadingContextLines() {
    if (this.model.getFindOptions().leadingContextLineCount > 0) {
      ...
      const searchContextLineCountBefore = lumine.config.get(
  ...
  toggleTrailingContextLines() {
    if (this.model.getFindOptions().trailingContextLineCount > 0) {
      ...
      const searchContextLineCountAfter = lumine.config.get(
